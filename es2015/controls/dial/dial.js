...
class Dial extends Component {
  ...
  render () {
    ...
    let stepperProps = { label, value, min, max, onChange }
    ...
    let a = [ circumference * value * range, circumference ].join(' ')
    let b = [ circumference, circumference ].join(' ')
    let transform = 'rotate(' + String(((1.0 - range) * 0.5) * 360 + 90) + 'deg )'
    ...
    */
    ...
    return <div style={[base, style, {height: 'auto'}]}>
      <NumericStepper { ...stepperProps }/>
        <svg style={[svgStyle, {transform}]} width={style.width} height={style.width} xmlns='http://www.w3.org/2000/svg' ref={ref => this.domRef = ref} onMouseDown={this.onMouseDown} onTouchStart={this.onMouseDown}>
